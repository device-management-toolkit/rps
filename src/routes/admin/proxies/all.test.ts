...
describe('Proxy - All', () => {
  ...
  it('should get all', async () => {
    ...
    expect(req.db.proxiesProfiles.get).toHaveBeenCalled()
    expect(resSpy.status).toHaveBeenCalledWith(200)
  })

  it('should get all with proxy configs length > 0', async () => {
    req.query.$count = true
    spyOn(req.db.proxiesProfiles, 'get').mockResolvedValue(['abc'])
    await allProxyProfiles(req, resSpy)
    expect(req.db.proxiesProfiles.get).toHaveBeenCalled()
    expect(resSpy.status).toHaveBeenCalledWith(200)
  })
  ...
  it('should get all with req.query.$count as true', async () => {
    ...
    await allProxyProfiles(req, resSpy)
    expect(req.db.proxiesProfiles.get).toHaveBeenCalled()
    expect(req.db.proxiesProfiles.getCount).toHaveBeenCalled()
    expect(resSpy.status).toHaveBeenCalledWith(200)
  })

  it('should set status to 500 if error occurs', async () => {
    req.db.proxiesProfiles.getCount = jest.fn().mockImplementation(() => {
