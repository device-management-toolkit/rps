...
export async function editProxyProfile(req: Request, res: Response): Promise<void> {
  ...
  try {
    const oldProxy: ProxyConfig | null = await req.db.proxyConfigs.getByName(newProxy.name, req.tenantId)

    if (oldProxy == null) {
      throw new RPSError(NOT_FOUND_MESSAGE('Proxy', newProxy.name), NOT_FOUND_EXCEPTION)
    } else {
      ...
      if (results) {
        MqttProvider.publishEvent('success', ['editProxyConfig'], `Updated proxy configuration : ${newProxy.name}`)
...
export const getUpdatedData = async (newProxy: ProxyConfig, oldProxy: ProxyConfig): Promise<ProxyConfig> => {
  const proxyConfig: ProxyConfig = { name: newProxy.name } as ProxyConfig
  // Determine the final address - either new or old
  const finalAddress = newProxy.address ?? oldProxy.address

  proxyConfig.address = finalAddress
  // Auto-detect infoFormat based on the final address
  proxyConfig.infoFormat = detectAddressFormat(finalAddress)
