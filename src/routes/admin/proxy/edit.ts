...
export async function editProxyProfile(req: Request, res: Response): Promise<void> {
  ...
  try {
    const oldProxy: ProxyConfig | null = await req.db.proxyConfigs.getByName(newProxy.address, req.tenantId)

    if (oldProxy == null) {
      throw new RPSError(NOT_FOUND_MESSAGE('AMT', newProxy.address), NOT_FOUND_EXCEPTION)
    } else {
      ...
      if (results) {
        MqttProvider.publishEvent('success', ['editProxyConfig'], `Updated proxy configuration : ${newProxy.address}`)
...
export const getUpdatedData = async (newProxy: ProxyConfig, oldProxy: ProxyConfig): Promise<ProxyConfig> => {
  const proxyConfig: ProxyConfig = { address: newProxy.address } as ProxyConfig
