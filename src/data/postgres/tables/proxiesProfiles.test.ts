...
import { ProxiesProfilesTable } from './proxiesProfiles.js'
import { RPSError } from '../../../utils/RPSError.js'
import { jest } from '@jest/globals'
import { type SpyInstance, spyOn } from 'jest-mock'

describe('proxies profiles tests', () => {
  let db: PostgresDb
  let proxiesProfilesTable: ProxiesProfilesTable
  ...
  beforeEach(() => {
    ...
    db = new PostgresDb('')
    proxiesProfilesTable = new ProxiesProfilesTable(db)
    querySpy = spyOn(proxiesProfilesTable.db, 'query')
  })
  afterEach(() => {
    jest.clearAllMocks()
  })
  describe('Get', () => {
    test('should get expected count', async () => {
      ...
      const count: number = await proxiesProfilesTable.getCount()
    ...
    test('should Get', async () => {
      ...
      const result = await proxiesProfilesTable.get()
    ...
    test('should get by name', async () => {
      ...
      const result = await proxiesProfilesTable.getByName(profileName)
    ...
    test('should check profile exist', async () => {
      querySpy.mockResolvedValueOnce({ rows: [], rowCount: 0 })
      const result = await proxiesProfilesTable.checkProfileExits(profileName)
      expect(querySpy).toBeCalledTimes(1)
      expect(querySpy).toBeCalledWith(
        `
    SELECT 1
  ...
  describe('Delete', () => {
    test('should delete', async () => {
      ...
      const result = await proxiesProfilesTable.delete(profileName)
    ...
    test('should NOT delete when relationship still exists to profile', async () => {
      querySpy.mockResolvedValueOnce({ rows: [], rowCount: 1 })
      await expect(proxiesProfilesTable.delete(profileName)).rejects.toThrow(
  ...
  describe('Insert', () => {
    test('should insert', async () => {
      querySpy.mockResolvedValueOnce({ rows: [], rowCount: 1 })
      const getByNameSpy = spyOn(proxiesProfilesTable, 'getByName')
      getByNameSpy.mockResolvedValue(proxyConfig)
      const result = await proxiesProfilesTable.insert(proxyConfig)
    ...
    test('should return null if insert does not return any rows (should throw an error though)', async () => {
      querySpy.mockResolvedValueOnce({ rows: [], rowCount: 0 })
      let result = await proxiesProfilesTable.insert(proxyConfig)
      expect(result).toBeNull()

      querySpy.mockResolvedValueOnce(null)
      result = await proxiesProfilesTable.insert(proxyConfig)
      expect(result).toBeNull()
    })
    test('should NOT insert when duplicate name', async () => {
      querySpy.mockRejectedValueOnce({ code: '23505' })
      await expect(proxiesProfilesTable.insert(proxyConfig)).rejects.toThrow(
  ...
  describe('Update', () => {
    test('should Update', async () => {
      ...
      const getByNameSpy = spyOn(proxiesProfilesTable, 'getByName')
      getByNameSpy.mockResolvedValue(proxyConfig)
      const result = await proxiesProfilesTable.update(proxyConfig)
      expect(result).toBe(proxyConfig)
      expect(getByNameSpy).toHaveBeenCalledWith(proxyConfig.proxyName, proxyConfig.tenantId)
      expect(querySpy).toBeCalledTimes(1)
      expect(querySpy).toBeCalledWith(
    ...
    test('should throw RPSError with no results from update query', async () => {
      ...
      const getByNameSpy = spyOn(proxiesProfilesTable, 'getByName')
      getByNameSpy.mockResolvedValue(proxyConfig)
      await expect(proxiesProfilesTable.update(proxyConfig)).rejects.toBeInstanceOf(RPSError)
    })
    ...
    test('should NOT update when unexpected error', async () => {
      ...
      const getByNameSpy = spyOn(proxiesProfilesTable, 'getByName')
      getByNameSpy.mockResolvedValue(proxyConfig)
      await expect(proxiesProfilesTable.update(proxyConfig)).rejects.toThrow(
        NETWORK_CONFIG_ERROR('Proxy', proxyConfig.proxyName)
      )
    })
    ...
    test('should NOT update when concurrency issue', async () => {
      ...
      const getByNameSpy = spyOn(proxiesProfilesTable, 'getByName')
      getByNameSpy.mockResolvedValue(proxyConfig)
      await expect(proxiesProfilesTable.update(proxyConfig)).rejects.toThrow(CONCURRENCY_MESSAGE)
      expect(getByNameSpy).toHaveBeenCalledWith(proxyConfig.proxyName, proxyConfig.tenantId)
      expect(querySpy).toBeCalledTimes(1)
      expect(querySpy).toBeCalledWith(
        `
