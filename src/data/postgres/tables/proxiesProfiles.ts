...
export class ProxiesProfilesTable implements IProxiesProfilesTable {
   ...
   */
  async getByName(configName: string, tenantId = ''): Promise<ProxyConfig | null> {
    const results = await this.db.query<ProxyConfig>(
      `
    SELECT 
      proxy_profile_name as "profileName",
  ...
  async delete(configName: string, tenantId = ''): Promise<boolean> {
    ...
    if (profiles?.rowCount) {
      if (profiles.rowCount > 0) {
        throw new RPSError(NETWORK_CONFIG_DELETION_FAILED_CONSTRAINT('Proxy', configName), 'Foreign key violation')
      }
    }
    try {
      const results = await this.db.query(
      ...
      this.log.error(`Failed to delete proxy configuration : ${configName}`, error)
      if (error.code === PostgresErr.C23_FOREIGN_KEY_VIOLATION) {
        throw new RPSError(NETWORK_CONFIG_DELETION_FAILED_CONSTRAINT('Proxy', configName))
      }
      throw new RPSError(API_UNEXPECTED_EXCEPTION(`Delete proxy configuration : ${configName}`))
    }
    return false
  }
