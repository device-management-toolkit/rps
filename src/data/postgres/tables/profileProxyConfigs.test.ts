...
describe('profileproxyconfig tests', () => {
  ...
  const proxiesConfigs: ProfileProxyConfigs[] = [{ profileName: 'proxyConfig', priority: 1 } as any]
  ...
  describe('Insert', () => {
    ...
    test('should NOT insert when constraint violation', async () => {
      ...
      await expect(profilesProxiesConfigsTable.createProfileProxyConfigs(proxiesConfigs, profileName)).rejects.toThrow('error')
    })
