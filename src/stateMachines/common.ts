...
/**
 * Computes the overall activation roll-up from the per-component results: any failure makes the
 * whole outcome a Failure; otherwise any success makes it a Success; otherwise NotApplicable.
 */
export function rollUpResult(components: ComponentResults): ComponentResultStatus {
  const results = Object.values(components).map((c) => c?.Result)
 ...
 * backfills Activation for already-activated devices that skipped the activation steps, fills every
 * un-attempted component with a NotApplicable entry so the key set is always complete, and stamps the
 * overall Result roll-up and Components schema version. Additive only — legacy flat fields are untouched.
 */
export function finalizeComponentResults(clientId: string, overallSuccess: boolean): void {
  ...
  // A failed outcome must never roll up to 'Success'.
  status.ComponentsRollup = overallSuccess ? rollUpResult(status.Components) : 'Failure'
  status.ComponentsVersion = COMPONENTS_VERSION
}
